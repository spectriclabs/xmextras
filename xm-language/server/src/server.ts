...
async function getDocumentSettings(resource: string): Promise<XmidasSettings> {
  ...
  for (let path of [...settings.xmDiskPaths].reverse()) {
    ...
    if (!foundManifest) {
      console.error(
        `${path} is not a valid XMIDAS path it will not be scanned because no manifest was found`,
      );
    }
  }
...
connection.onHover(async (params: HoverParams, ...rest) => {
  console.log(params, rest);
  let document = documents.get(params.textDocument.uri);
  if (document?.languageId) {
    console.log(`Detected language ${document?.languageId}`);
  }
  ...
  if (commands.length) {
    ...
    let typedef = `${command.name.toLowerCase()} ${command.params
      .map((param) => {
        const [typeName, type] = Object.entries(XMParamType).filter(
          (e) => e[1] === param.type,
        )[0];
...
function getTokenFromText(params: TextDocumentPositionParams) {
  ...
  if (document.languageId == "xmidas") {
    ...
    let startIndex = line.indexOf(text);
    let tokenStart = line.lastIndexOf("xm.", startIndex) + 3;
    let tokenEnd = line.indexOf("(", tokenStart);
    if (tokenEnd == -1) {
      //check if we are typing a new command on an existing line
      tokenEnd = line.indexOf("\n", tokenStart);
...
function parseCommands(text: string, path: string) {
  ...
  const pattern =
    /(\d+)?\s+(\w+)\*(\w+)?\s+(\w),(\d+)(\+?)\s+(.+)+#([IOANRTUHCKDFXLIBO_?\d]+)/g;
  ...
  for (let match of text.matchAll(pattern)) {
    ...
    if (SUPPORT_TYPE_TO_FOLDER[supportType]) {
      ...
      files = files.filter((f) => !f.match(/.*(\.exe|\.so|\.mcr|\.b|\.set)/));
      if (files.length == 1) {
        location = files[0];
      }
    }
    ...
    for (let match of paramTypes.matchAll(/(\d+\w|\w)/g)) {
      ...
      if (count == "") {
        //There is only one of this type in the sequence
        command.params.push({
...
async function getCommandsFromText(params: TextDocumentPositionParams) {
  ...
  for (let path of [...settings.xmDiskPaths]) {
    ...
    commands = commands.filter((command) => {
      return (
        token.toLowerCase().startsWith(command.shortName.toLowerCase()) ||
  ...
  commands = commands.sort(
    (a, b) =>
      levDist(a.name.toLowerCase(), token) -
      levDist(b.name.toLowerCase(), token),
  );
  return commands;
}
...
connection.onCompletion(async (params, cancel) => {
  ...
  if (
    !document ||
    (document.languageId !== "python" && document.languageId != "xmidas")
  ) {
    return;
  }
  ...
  commands.forEach((command) => {
    ...
    if (document!.languageId === "python") {
      let parameters = command.params.map((param) => {
        if (param.type === XMParamType.ASCII) {
      ...
      insertText = `${command.name.toLowerCase()}(${parameters.join(", ")})`;
    } else {
      insertText = `${command.name.toLowerCase()} ${command.params
        .map((param) => {
          return param.default;
        })
        .join(",")}`;
    ...
    completions.items.push({
      ...
      detail: command.params
        .map((param) => {
          return Object.entries(XMParamType)
            .filter((entry) => entry[1] === param.type)
            .map((entry) => entry[0]);
        })
        .join(","),
